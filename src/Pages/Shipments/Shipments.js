...
const Shipment = () => {
    ...
    const paginated = filteredShipments?.slice(startIndex, startIndex + pageSize);
        ...
        axios.get(URL, {
            ...
            .then((response) => {
                if (response?.status === 200) {
                    setShipmentDetails(response?.data?.shipmentList);
                    setPageCount(response?.data?.totalShipmentCount);
                    setLoading(false);
                }
            })
            .catch((error) => {
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    const response = await axios.delete(
                        `${environment.config.apiBaseUri}shipment/deleteShipment/${item?.id}`,
                        {
                            headers: {
                                authorization: `Bearer ${parsedToken}`,
                                'Content-Type': 'application/json'
    ...
    return (
        <div>
            ...
            {showModal && <Modal modalType={'Shipments'} show={showModal} isEdit={isEdit} editData={editData} getAllShipments={getAllShipments} parsedToken={parsedToken} onClose={() => { setShowModal(false); setIsEdit(false); setEditData([]) }} />}

            <div className="card">
                <table>
                    <thead>
                        <tr>
                            <th>Shipment Type</th><th>Source</th><th>Destination</th><th>Vehicle</th><th>RFID Tag</th>
                            <th>Driver Name</th><th>Driver Mobile</th><th>Status</th><th>Edit</th><th>Delete</th>
                        </tr>
                    </thead>
                    <tbody>
                        {filteredShipments?.map((s) => (
                            <tr key={s.id} className={s.id === newRowId ? 'row-new' : ''}>
                                <td className="cell-primary num">{s.shipmentType}</td>
                                <td>{s.sourcePlant}</td>
                                <td>{s.destinationPlant}</td>
                                <td className="num">{s.vehicleNo}</td>
                                <td className="num">{s.rfidTag}</td>
                                <td>{s.driverName}</td>
                                <td className="num">{s.driverMobile}</td>
                                {/* <td><span className={`tag tag-${s.tag}`}><span className="tag-dot" />{s.status}</span></td> */}
                                ...
                                {/* <td>
                                    <button className="btn btn-ghost btn-sm" onClick={() => handleTrackShipment(s)}>
                                        Track
                                    </button>
                                </td> */}
                                {/* <td className="d-md-table-cell align-middle py-3 px-2">
                                    <div className="dropdown" style={{ position: 'static' }}>
                                        <button
                                            className="btn btn-light btn-sm dropdown-toggle action-btn"
                                            data-bs-toggle="dropdown"
                                        >
                                            Actions
                                        </button>
                                        <ul
                                            className="dropdown-menu dropdown-menu-end shadow-sm border-0 rounded-3 p-2 action-menu"
                                            style={{ minWidth: '180px', zIndex: 9999 }}
                                        >
                                            <li>
                                                <button
                                                    className="dropdown-item action-item rounded-2"
                                                    onClick={() => {
                                                        setIsEdit(true);
                                                        setShowModal(true);
                                                        setEditData(s);
            ...
            <div className="pagination-bar">
                <div className="pagination-info">
                    <span>
                        ...
                        {Math.min(startIndex + pageSize, filteredShipments.length)} of{" "}
